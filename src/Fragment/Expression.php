...
final class Expression implements Stringable, Countable, IteratorAggregate
{
    ...
    /**
     * Returns the TabularDataReader from the first selection if it exists otherwise it returns null
     *
     * @throws Exception
    ...
    /**
     * Fragment the TabularDataReader instance for each selection.
     *
     * @throws Exception
     * @throws InvalidArgument
     * @throws ReflectionException
    ...
    public function fragment(TabularDataReader $tabularDataReader): iterable
    {
        ...
        foreach ($this->selections as $selection) {
            yield $this->type->value.'='.$selection->toString() => $selection->query($nbColumns)->process($tabularDataReader);
        }
    }

    /**
     * Fragment and merge the TabularData selections if possible.
     *
     * @throws Exception
     * @throws InvalidArgument
     * @throws ReflectionException
    ...
    {
        foreach ($this->selections as $selection) {
            yield $this->type->value.'='.$selection->toString() => $selection->query($nbColumns);
        }
    }
     ...
     * @return iterable<string, Statement>
     */
    private function queryByColumns(int $nbColumns = 0): iterable
    {
