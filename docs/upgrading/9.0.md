...
The following methods are no longer attached to the `Reader` class instead they are expose using the `ResultSet` object. They also no longer accept an optional callable as argument.
